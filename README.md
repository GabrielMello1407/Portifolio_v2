...
- **Terminal interativo** — abre com `/` ou `⌘K` (e por um botão na navbar). Não é só um menu: é um **shell de mentira com filesystem** — `ls`, `cd`, `cat`, `pwd`, `tree`, e até `mkdir`/`touch`/`rm` (na sua sessão). Os projetos viram arquivos navegáveis e há uma pasta escondida pra achar.
